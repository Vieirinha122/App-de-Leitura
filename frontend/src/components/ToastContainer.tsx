...
function ToastItem({ toast, onClose }: { toast: Toast; onClose: (id: string) => void }) {
  const icons = {
    ...
    info: (
      <svg className="h-5 w-5 text-sage-600" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
  ...
  const bgColors = {
    ...
    info: 'bg-sage-50 border-sage-200'
  }

  return (
    <div
      className={`pointer-events-auto flex items-start gap-3 w-full max-w-sm rounded-lg border p-4 shadow-elevated animate-slide-up ${bgColors[toast.type]}`}
      role="alert"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {toast.message && (
          <p className="mt-1 text-body-sm text-ink-600">{toast.message}</p>
        )}
      </div>
      <button
        type="button"
        className="flex-shrink-0 p-1 rounded text-ink-400 hover:text-ink-600 hover:bg-ink-100 transition-colors"
